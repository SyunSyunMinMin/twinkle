...
Morebits.wiki.api.prototype = {
		...
		return $.ajax(ajaxparams).then(
			...
			function onAPIsuccess(response, statusText) {
				...
				if (this.onSuccess) {
					...
					this.statelem.info(msg('done', '完了'));
				}
	...
	returnError: function(callerAjaxParameters) {
		if (this.errorCode === 'badtoken' && !this.badtokenRetry) {
			this.statelem.warn(msg('invalid-token-retrying', 'トークンが無効です。新しいトークンを取得して再試行します...'));
...
Morebits.wiki.api.getToken = function() {
	var tokenApi = new Morebits.wiki.api(msg('getting-token', 'トークンを取得'), {
		action: 'query',
		meta: 'tokens',
		type: 'csrf',
		format: 'json'
...
Morebits.wiki.page = function(pageName, status) {
	...
	if (!status) {
		status = msg('opening-page', pageName, '"' + pageName + '"を開く');
	}
			...
			!confirm(
				ctx.fullyProtected === 'infinity'
					? msg('protected-indef-edit-warning', ctx.pageName,
						'全保護されたページ "' + ctx.pageName + '" (無期限保護) を編集しようとしています。\n\n編集を続行するにはOKをクリックし、この編集をスキップするにはキャンセルをクリックしてください。'
					)
					: msg('protected-edit-warning', ctx.pageName, ctx.fullyProtected,
						'全保護されたページ "' + ctx.pageName +
					'" (保護期限: ' + new Morebits.date(ctx.fullyProtected).calendar('utc') + ' (UTC)) を編集しようとしています。\n\n編集を続行するにはOKをクリックし、この編集をスキップするにはキャンセルをクリックしてください。'
					)
			)
		) {
			ctx.statusElement.error(msg('protected-aborted', '全保護されたページの編集が中断されました。'));
			ctx.onSaveFailure(this);
			return;
		}
	...
	var fnLoadSuccess = function() {
		...
		if (!ctx.csrfToken) {
			ctx.statusElement.error(msg('token-fetch-fail', '編集トークンの取得に失敗しました。'));
			ctx.onLoadFailure(this);
			return;
		}
		ctx.loadTime = ctx.loadApi.getResponse().curtimestamp;
		if (!ctx.loadTime) {
			ctx.statusElement.error('現在のタイムスタンプの取得に失敗しました。');
			ctx.onLoadFailure(this);
			return;
		}
	...
	var fnCheckPageName = function(response, onFailure) {
		...
		if (page) {
			...
			if (page.invalid) {
				ctx.statusElement.error(msg('invalid-title', ctx.pageName, '無効なページタイトル: ' + ctx.pageName));
				onFailure(this);
				return false; // abort
			}
			...
			if (response.redirects) {
				...
				if (origNs !== newNs && !ctx.followCrossNsRedirect) {
					ctx.statusElement.error(msg('cross-redirect-abort', ctx.pageName, resolvedName, ctx.pageName + 'は' + resolvedName + 'への名前空間が違うリダイレクトです。中止します。'));
					onFailure(this);
					return false;
				}

				// only notify user for redirects, not normalization
				new Morebits.status('Note', msg('redirected', ctx.pageName, resolvedName, ctx.pageName + 'から' + resolvedName + 'へのリダイレクト'));
			}
	...
	var fnSaveSuccess = function() {
		...
		if (response.edit.captcha) {
			ctx.statusElement.error('ウィキのサーバーがキャプチャを入力するよう要求したため、ページを保存できませんでした。');
		} else {
			ctx.statusElement.error(msg('api-error-unknown', 'ページの保存中にAPIから不明なエラーを受信'));
		}
	...
	var fnSaveError = function() {
		...
		if (errorCode === 'editconflict' && ctx.conflictRetries++ < ctx.maxConflictRetries) {
			...
			var purgeApi = new Morebits.wiki.api(msg('editconflict-purging', '編集の競合が検出されたため、サーバーのキャッシュを破棄します。'), purgeQuery, function() {
				--Morebits.wiki.numberOfActionsLeft;  // allow for normal completion if retry succeeds

				ctx.statusElement.info(msg('editconflict-retrying', '編集の競合が検出されたため、編集をやり直します'));
				if (fnCanUseMwUserToken('edit')) {
		...
		// check for network or server error
			...
			ctx.statusElement.info(msg('save-failed-retrying', 2, '保存に失敗しました。2秒後に再試行します ...'));
			--Morebits.wiki.numberOfActionsLeft;  // allow for normal completion if retry succeeds
	...
	var fnProcessMove = function() {
		...
		ctx.moveProcessApi = new Morebits.wiki.api(msg('moving-page', 'ページの移動...'), query, ctx.onMoveSuccess, ctx.statusElement, ctx.onMoveFailure);
...
Morebits.batchOperation = function(currentAction) {
	var ctx = {
		...
		statusElement: new Morebits.status(currentAction || msg('batch-starting', 'バッチ処理の実行')),
	...
	this.run = function(worker, postFinish) {
		...
		if (!total) {
			ctx.statusElement.info(msg('batch-no-pages', 'ページが指定されていません'));
			ctx.running = false;
			if (ctx.postFinish) {
				ctx.postFinish();
			}
